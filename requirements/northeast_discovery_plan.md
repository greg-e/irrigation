...
| Pattern C "Suggested Repairs" review screen at checkout | [requirements/fsm_capability_validation.md](fsm_capability_validation.md) §4.1 | IrrigationCheckups auto-builds quotes during the checkup, not at checkout. NE techs may expect inline quote-build, not end-of-visit review. |
| PDF report content + style | [requirements/fsm_capability_validation.md](fsm_capability_validation.md) §4.2 | Use NE's IrrigationCheckups PDFs as reference benchmark. |
| Per-zone grid UX (mobile) | [requirements/fsm_capability_validation.md](fsm_capability_validation.md) §2.9 | NE will say whether IrrigationCheckups' per-zone UX (which is well-regarded) sets the bar — or if our card-based design is acceptable. |
